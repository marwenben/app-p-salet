...
// Ajouter le sélecteur de langue pour le Coran
let quranDisplayMode = 'bilingual'; // 'arabic', 'french', 'bilingual'

// Modifier la fonction de chargement de sourate existante
const originalLoadSurahBtn = document.getElementById('load-surah-btn');
if (originalLoadSurahBtn) {
    // Ajouter un sélecteur de mode d'affichage avant le bouton
    const quranControls = originalLoadSurahBtn.parentElement;
    const modeSelector = document.createElement('div');
    modeSelector.className = 'quran-mode-selector';
    modeSelector.style.cssText = 'margin: 15px 0; text-align: center;';
    modeSelector.innerHTML = `
        <label style="display: block; margin-bottom: 10px; color: #1e3c72; font-weight: 600;">
            <span data-fr="Mode d'affichage:" data-ar="نمط العرض:">Mode d'affichage:</span>
        </label>
        <div style="display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
            <button id="mode-arabic" class="mode-btn active" style="flex: 1; min-width: 120px; padding: 10px; border: 2px solid #1e3c72; background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: white; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                <span data-fr="Arabe seul" data-ar="عربي فقط">Arabe seul</span>
            </button>
            <button id="mode-french" class="mode-btn" style="flex: 1; min-width: 120px; padding: 10px; border: 2px solid #1e3c72; background: white; color: #1e3c72; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                <span data-fr="Français seul" data-ar="فرنسي فقط">Français seul</span>
            </button>
            <button id="mode-bilingual" class="mode-btn" style="flex: 1; min-width: 120px; padding: 10px; border: 2px solid #1e3c72; background: white; color: #1e3c72; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                <span data-fr="Arabe + Français" data-ar="عربي + فرنسي">Arabe + Français</span>
            </button>
        </div>
    `;
    
    quranControls.insertBefore(modeSelector, originalLoadSurahBtn);
    
    // Événements pour les boutons de mode
...
if (originalLoadSurahBtn) {
    ...
    const newLoadBtn = originalLoadSurahBtn.cloneNode(true);
    originalLoadSurahBtn.parentNode.replaceChild(newLoadBtn, originalLoadSurahBtn);
    ...
    newLoadBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Charger la traduction française (Hamidullah)
